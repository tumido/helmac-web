...
empty-block-drop-zone.tsx   pseudo-row rendered by the canvas right after every empty block
                            (block with no children below). Registers useDroppable
                            "container-<blockId>" so resolveDropTarget maps it to
                            {parent: blockId, index: 0}. Visually a block-tail wrapper with
                            an inner dashed "Přetáhněte pole sem" placeholder.
